const addSelect = (title, options, selected) => {
    const optionList = options.map((option) => option === selected ? `<option $ ? selected="selected">${option}</option>` : `<option>${option}</option>`)
...
const blanks = {
    ...
    osb24: {
        ...
        height: 1220,
        image: 'url(img/wood.jpg)',
        title: addSelect('Плита OSB 2440 × 1220', [9, 12, 15], 12)
    },
    playWood: {
        width: 1525,
        height: 1525,
    ...
    dspGroove600: {
        ...
        height: 600,
        image: 'url(img/dsp.jpg)',
        title: addSelect('ДСП 2440х600 Quick Deck Professional шпунтованная', [16, 22], 16)
    },
    dspGroove900: {
        width: 2440,
        height: 900,
